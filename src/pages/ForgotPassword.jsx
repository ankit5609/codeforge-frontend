...
export default function ForgotPassword() {
    ...
    return (<AuthLayout eyebrow="// Recover access" heading={<>
          Forgot your{" "}
          <span style={{
        ...
        </>} description="Enter the email tied to your workspace and we'll send you a secure link to set a new password." badges={["one-time link", "expires in 15 minutes", "encrypted delivery"]}>
      <AuthCard>
        {sent ? (<div className="space-y-5">
            ...
            <p className="text-[13px]" style={{ color: "var(--lp-ink-faint)" }}>
              ...
              <button type="button" onClick={() => setSent(false)} className="font-semibold hover:underline" style={{ color: "var(--lp-brass)" }}>
                try another email
              </button>
              .
            </p>
            <Link to="/login" className="inline-flex items-center gap-2 text-[13px] font-semibold hover:underline" style={{ color: "var(--lp-brass)" }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-[16px]">
              <div>
                <label htmlFor="email" className="block font-mono uppercase mb-2" style={{
                fontSize: "10.5px",
                ...
                <FieldShell icon={<Mail className="w-4 h-4"/>}>
                  <input id="email" type="email" autoComplete="username" placeholder="you@company.com" value={email} onChange={(e) => {
                setEmail(e.target.value);
                if (fieldError)
                    setFieldError(null);
            }} disabled={isLoading} autoFocus className="lp-input"/>
                </FieldShell>
                {fieldError && (<p className="mt-2 text-[12px]" style={{ color: "#ff7a5c" }}>
                    {fieldError}
                  </p>)}
              </div>
