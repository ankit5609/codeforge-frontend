...
import {
    AlertTriangle, ArrowUpDown, Bell, Check, ChevronsUpDown, CreditCard, Download, Edit, FolderGit2,
    GitBranch, Import, LayoutGrid, LayoutTemplate, List, Loader2, LogOut, MoreVertical, Plus, Search,
    Settings as SettingsIcon, SlidersHorizontal, Sparkles, Trash, Users, X, Zap, Menu,
    Infinity as InfinityIcon, type LucideIcon,
} from "lucide-react";
...
import { ProjectSummaryResponse, SubscriptionResponse, ProjectRole } from "@/lib/types";
import { useToast } from "@/hooks/use-toast";
import { PRICING_PLANS, describeSubscription, type SubscriptionDisplay } from "@/lib/plans";
import { deriveTokenUsage, deriveProjectUsage, formatNumber, type DerivedUsage } from "@/lib/usage";
import { PROJECT_TEMPLATES } from "@/lib/templates";
import { generateGradient, cn } from "@/lib/utils";

/* ============================================================================
   ONE FILE, different shell than before: a persistent left sidebar (brand,
   greeting, compact ring-stats, primary actions, account) + a main content
   column that's just the toolbar and the projects grid. Same reasoning as
   last time — Linear/Notion/Vercel all use a persistent rail, not a stacked
   marketing-style page, so this is a structural change, not a re-skin.
   All state/handlers in ProjectsDashboard() are unchanged from the original.
============================================================================ */
...
type Tone = "ember" | "brass" | "teal" | "neutral";

const SORT_LABEL: Record<SortOption, string> = { newest: "Newest first", oldest: "Oldest first", name: "Name A–Z" };
const FILTER_LABEL: Record<RoleFilter, string> = { all: "All roles", OWNER: "Owner", EDITOR: "Editor", VIEWER: "Viewer" };
...
const PLAN_TONE: Record<SubscriptionDisplay["tone"], Tone> = { primary: "ember", amber: "brass", muted: "neutral" };
const PLAN_DOT: Record<SubscriptionDisplay["tone"], string> = { primary: "var(--lp-ember)", amber: "var(--lp-brass)", muted: "var(--lp-ink-faint)" };
...
function ProgressRing({ pct, size = 40, stroke = 3.5, color, unlimited }: { pct: number; size?: number; stroke?: number; color: string; unlimited?: boolean }) {
    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle
                ...
                style={{ transition: "stroke-dashoffset 0.8s cubic-bezier(.16,.8,.3,1)" }}
            />
        </svg>
    );
}
...
function EmptyIllustration() {
    return (
        <svg width="120" height="120" viewBox="0 0 132 132" fill="none" aria-hidden>
            <defs>
...
function SidebarStat({ icon: Icon, label, value, meta, pct, unlimited, tone }: {
    icon: LucideIcon; label: string; value: ReactNode; meta?: ReactNode; pct?: number; unlimited?: boolean; tone: Tone;
}) {
    ...
    return (
        <div className="flex items-center gap-3">
            {typeof pct === "number" ? (
                ...
                <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ background: c.bg, color: c.fg }}>
...
function Sidebar({
    firstName, planDisplay, tokenUsage, projectUsage, ownedCount, sharedCount,
    userName, userEmail, showUpgrade, isDemoLocked, isSubscribed, isPortalLoading,
    onNewProject, onTemplates, onImportClick, onUpgradeClick, onManageBilling, onSettings, onLogout,
    mobileOpen, onCloseMobile,
}: {
    firstName: string; planDisplay: SubscriptionDisplay; tokenUsage: DerivedUsage; projectUsage: DerivedUsage;
    ownedCount: number; sharedCount: number; userName?: string; userEmail?: string;
    showUpgrade: boolean; isDemoLocked: boolean; isSubscribed: boolean; isPortalLoading: boolean;
    onNewProject: () => void; onTemplates: () => void; onImportClick: () => void;
    onUpgradeClick: () => void; onManageBilling: () => void; onSettings: () => void; onLogout: () => void;
    ...
    return (
        <>
            ...
            <aside
                className={cn(
                    "fixed inset-y-0 left-0 z-50 w-[272px] flex flex-col transition-transform duration-300 lg:sticky lg:top-0 lg:z-auto lg:h-screen lg:translate-x-0",
                    mobileOpen ? "translate-x-0" : "-translate-x-full"
                )}
                style={{ background: "var(--lp-bg-raised)", borderRight: "1px solid var(--lp-border)" }}
            >
                <div className="flex-1 overflow-y-auto px-5 pt-6 pb-4">
                    ...
                    <div className="space-y-4 pb-6 mb-6" style={{ borderBottom: "1px solid var(--lp-border-soft)" }}>
                        <SidebarStat
                            icon={FolderGit2} label="Projects" tone="ember"
                            value={<AnimatedNumber value={projectUsage.used} />}
                            meta={projectUsage.limit > 0 ? `/ ${projectUsage.limit}` : undefined}
                            pct={projectUsage.limit > 0 ? projectUsage.pct : undefined}
                        />
                        <SidebarStat
                            icon={Sparkles} label="AI usage" tone="brass" unlimited={tokenUsage.unlimited}
                            value={tokenUsage.unlimited ? "Unlimited" : <AnimatedNumber value={tokenUsage.pct} formatter={(n) => `${n}%`} />}
                            pct={tokenUsage.unlimited ? 100 : tokenUsage.pct}
                        />
                ...
                <div className="shrink-0 p-3" style={{ borderTop: "1px solid var(--lp-border-soft)" }}>
                    {(showUpgrade || isDemoLocked) && (
                        <button onClick={onUpgradeClick} className="lp-btn lp-btn-solid !w-full !mb-2 !text-[13px]">
                            <Sparkles className="w-4 h-4" /> {isDemoLocked ? "View plans" : "Upgrade"}
                        </button>
                    )}
                    {isSubscribed && (
                        <button onClick={onManageBilling} disabled={isPortalLoading} className="lp-btn lp-btn-ghost !w-full !mb-2 !text-[13px]">
...
function MobileTopBar({ onOpenSidebar, onNotificationsClick }: { onOpenSidebar: () => void; onNotificationsClick: () => void }) {
    return (
        <div className="lg:hidden sticky top-0 z-30 flex items-center justify-between h-14 px-4 backdrop-blur-md" style={{ borderBottom: "1px solid var(--lp-border-soft)", background: "rgba(10,13,18,0.85)" }}>
...
function DashboardToolbar({ searchQuery, onSearchChange, sortBy, onSortChange, roleFilter, onRoleFilterChange, viewMode, onViewModeChange }: {
    ...
    roleFilter: RoleFilter; onRoleFilterChange: (v: RoleFilter) => void;
    ...
    return (
        <div className="flex flex-col sm:flex-row gap-2.5">
            <div className="relative flex-1 max-w-sm">
                ...
                <input
                    placeholder="Search projects..." value={searchQuery} onChange={(e) => onSearchChange(e.target.value)}
                    className="lp-input !pl-10" style={{ height: "40px", borderRadius: "10px" }}
                />
            </div>
            <div className="flex items-center gap-2">
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className={menuContentClass} style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                        {(Object.keys(SORT_LABEL) as SortOption[]).map((opt) => (
                            <DropdownMenuItem key={opt} onClick={() => onSortChange(opt)} className="cursor-pointer rounded-[8px] text-[13.5px] flex items-center justify-between" style={{ color: "var(--lp-ink)" }}>
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className={menuContentClass} style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                        {(Object.keys(FILTER_LABEL) as RoleFilter[]).map((opt) => (
                            <DropdownMenuItem key={opt} onClick={() => onRoleFilterChange(opt)} className="cursor-pointer rounded-[8px] text-[13.5px] flex items-center justify-between" style={{ color: "var(--lp-ink)" }}>
...
function SubscriptionBanner({ message, onUpgrade }: { message?: string | null; onUpgrade: () => void }) {
    return (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 0.8, 0.3, 1] }}
            className="flex items-start gap-4 rounded-[16px] p-5" style={{ border: "1px solid rgba(232,184,75,0.3)", background: "rgba(232,184,75,0.07)" }}>
...
function ProjectCard({ project, variant = "grid", onOpen, onRename, onDownload, onDelete }: {
    project: ProjectSummaryResponse; variant?: ViewMode;
    onOpen: () => void; onRename: (e: React.MouseEvent) => void; onDownload: (e: React.MouseEvent) => void; onDelete: (e: React.MouseEvent) => void;
}) {
    const roleStyle = project.role ? ROLE_STYLE[project.role] : null;
    const created = (() => {
        try { return `${formatDistanceToNow(new Date(project.createdAt))} ago`; } catch { return new Date(project.createdAt).toLocaleDateString(); }
    })();

    const avatar = (
        <div className="rounded-[14px] flex items-center justify-center shrink-0 overflow-hidden border border-white/10 shadow-md" style={{ width: variant === "grid" ? 48 : 44, height: variant === "grid" ? 48 : 44 }}>
            {project.thumbnailUrl ? (
                <img src={project.thumbnailUrl} alt="" className="w-full h-full object-cover" />
            ) : (
                <div className="w-full h-full flex items-center justify-center" style={generateGradient(project.name)}>
    ...
    const roleBadge = project.role && (
        <span className="text-[10px] font-semibold uppercase tracking-wider px-2 py-[3px] rounded-full border" style={{ background: roleStyle?.bg, color: roleStyle?.fg, borderColor: roleStyle?.border }}>{project.role}</span>
    );

    if (variant === "list") {
        return (
            <motion.div layout whileHover={{ y: -2 }} transition={{ duration: 0.18, ease: [0.16, 0.8, 0.3, 1] }} onClick={onOpen}
                className="group cursor-pointer rounded-[14px] p-4 flex items-center gap-4" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                {avatar}
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                    ...
                    {project.description ? (
                        <p className="text-[13px] truncate mt-0.5" style={{ color: "var(--lp-ink-faint)" }}>{project.description}</p>
                    ) : (
                        <span className="flex items-center gap-1 text-[12px] mt-0.5" style={{ color: "var(--lp-ink-faint)" }}><GitBranch className="w-3 h-3" /> main</span>
                    )}
                </div>
    ...
    return (
        <motion.div layout whileHover={{ y: -4 }} transition={{ duration: 0.2, ease: [0.16, 0.8, 0.3, 1] }} onClick={onOpen}
            className="group cursor-pointer rounded-[18px] overflow-hidden flex flex-col p-5"
            style={{ background: "linear-gradient(180deg, var(--lp-bg-raised) 0%, #0F131A 100%)", border: "1px solid var(--lp-border)", boxShadow: "0 4px 0 0 rgba(0,0,0,0)" }}
            onMouseEnter={(e) => (e.currentTarget.style.boxShadow = "0 28px 54px -26px rgba(0,0,0,0.7)")}
...
function ProjectCardSkeleton() {
    return (
        <div className="rounded-[18px] p-5 flex flex-col" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
            <div className="flex items-start gap-3">
...
function EmptyProjectsState({ isSearching, onCreateProject }: { isSearching: boolean; onCreateProject: () => void }) {
    return (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 0.8, 0.3, 1] }}
            className="text-center py-16 px-8 rounded-[20px] max-w-lg mx-auto" style={{ border: "1px dashed var(--lp-border)", background: "var(--lp-bg-raised)" }}>
...
function CreateProjectDialog({ open, onOpenChange, projectName, onProjectNameChange, onCreate, isCreating }: {
    open: boolean; onOpenChange: (open: boolean) => void; projectName: string; onProjectNameChange: (v: string) => void; onCreate: () => void; isCreating: boolean;
}) {
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent
                className="landing-scope !w-[min(360px,calc(100vw-32px))] !max-w-none !h-auto !min-h-0 !max-h-[calc(100vh-80px)] !gap-0 !rounded-[12px] !p-4 overflow-hidden"
                style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)", display: "flex", flexDirection: "column" }}
            >
                    ...
                    <div>
                        ...
                        <div className="mt-1 grid grid-cols-3 gap-1.5">
                            {PROJECT_TEMPLATES.map((tpl) => {
                                const Icon = tpl.icon;
                                return (
                                    <button key={tpl.id} type="button" onClick={() => { if (tpl.id !== "blank" && !projectName.trim()) onProjectNameChange(tpl.name); }}
                                        className="group text-left rounded-[8px] px-2 py-1.5 transition-all hover:-translate-y-0.5" style={{ border: "1px solid var(--lp-border)", background: "var(--lp-bg-raised-2)" }}
                                        onMouseEnter={(e) => (e.currentTarget.style.borderColor = "var(--lp-ember)")} onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--lp-border)")}>
...
function RenameProjectDialog({ open, onOpenChange, renameName, onRenameNameChange, onSubmit, originalName }: {
    open: boolean; onOpenChange: (open: boolean) => void; renameName: string; onRenameNameChange: (v: string) => void; onSubmit: () => void; originalName?: string;
}) {
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
...
interface PlanTier { planId: number; name: string; price: string; period: string; tagline: string; recommended: boolean; features: string[]; }

function PlanDialog({ open, onOpenChange, tiers, checkoutPlanId, onChoosePlan }: {
    open: boolean; onOpenChange: (open: boolean) => void; tiers: PlanTier[]; checkoutPlanId: number | null; onChoosePlan: (planId: number) => void;
}) {
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="landing-scope sm:max-w-2xl rounded-[18px] p-6" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
                    {tiers.map((tier) => {
                        ...
                        return (
                            <div key={tier.planId} className="relative rounded-[16px] p-6 flex flex-col" style={{ background: "var(--lp-bg-raised-2)", border: tier.recommended ? "1px solid var(--lp-ember)" : "1px solid var(--lp-border)", boxShadow: tier.recommended ? "0 20px 50px -24px var(--lp-ember-glow)" : "none" }}>
                                ...
                                <ul className="mt-5 space-y-2.5 flex-1">
                                    {tier.features.map((feature) => (
                                        <li key={feature} className="flex items-start gap-2 text-[13.5px]" style={{ color: "var(--lp-ink-dim)" }}><Check className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "var(--lp-teal)" }} /> {feature}</li>
                                    ))}
                                </ul>
...
export function ProjectsDashboard() {
    ...
    const fetchSubscription = async () => {
        try { const data = await api.getCurrentSubscription(); setSubscription(data); }
    ...
    const handleManageBilling = async () => {
        ...
        if (!active) { setIsPlanDialogOpen(true); return; }
    ...
    const handleRenameClick = (e: React.MouseEvent, project: ProjectSummaryResponse) => {
        e.stopPropagation();
        setProjectToRename(project); setRenameName(project.name); setIsRenameDialogOpen(true);
    };
    ...
    const handleRenameSubmit = async () => {
        ...
        try {
            ...
            setIsRenameDialogOpen(false); setProjectToRename(null);
    ...
    const sortedProjects = [...filteredProjects].sort((a, b) => {
        ...
        const at = new Date(a.createdAt).getTime(); const bt = new Date(b.createdAt).getTime();
    ...
    return (
        <div className="landing-scope min-h-screen w-full flex" style={{ background: "var(--lp-bg)" }}>
            ...
            <Sidebar
                firstName={firstName} planDisplay={planDisplay} tokenUsage={tokenUsage} projectUsage={projectUsage}
                ownedCount={ownedCount} sharedCount={sharedCount} userName={user?.name} userEmail={user?.username}
                showUpgrade={showUpgrade} isDemoLocked={isDemoLocked} isSubscribed={isSubscribed} isPortalLoading={isPortalLoading}
                onNewProject={() => setIsDialogOpen(true)} onTemplates={handleTemplatesClick} onImportClick={handleImportClick}
                onUpgradeClick={() => setIsPlanDialogOpen(true)} onManageBilling={handleManageBilling}
                onSettings={() => navigate("/settings")} onLogout={handleLogout}
            ...
            <div className="relative z-10 flex-1 min-w-0 flex flex-col">
                ...
                <main className="flex-1 w-full px-5 sm:px-8 xl:px-10 py-7 xl:py-9 flex flex-col gap-6 max-w-[1400px]">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={handleNotificationsClick} className="hidden lg:flex w-9 h-9 items-center justify-center rounded-full transition-colors" style={{ color: "var(--lp-ink-faint)" }}
                            onMouseEnter={(e) => (e.currentTarget.style.background = "var(--lp-bg-raised)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                            <Bell className="w-[18px] h-[18px]" />
                        </button>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <EmptyProjectsState isSearching={!!searchQuery} onCreateProject={() => setIsDialogOpen(true)} />
                    ) : (
                        <motion.div layout className={viewMode === "grid" ? "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5" : "flex flex-col gap-3"}>
                            <AnimatePresence initial={false}>
                                {visibleProjects.map((project) => (
                                    <ProjectCard
                                        key={project.id} project={project} variant={viewMode}
