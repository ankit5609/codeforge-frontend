...
export default function Success() {
    ...
    const periodEnd = subscription?.currentPeriodEnd
        ...
        : null;
    return (<div className="min-h-screen bg-background relative overflow-hidden flex items-center justify-center px-6 py-16 animate-fade-in">
      {/* Ambient background */}
        ...
        <div className="rounded-3xl border border-border/70 bg-card/80 backdrop-blur-md p-9 sm:p-11 text-center shadow-[0_30px_60px_-24px_hsl(156_50%_2%/0.7)]">
          {loading ? (<>
              ...
              <div className="mt-6 flex items-center justify-center gap-2">
                {Array.from({ length: MAX_ATTEMPTS }).map((_, i) => (<span key={i} className={cnDot(i < attempt)}/>))}
              </div>
              ...
              <dl className="text-left rounded-2xl border border-border/60 bg-background/40 divide-y divide-border/50 overflow-hidden mb-6">
                ...
                <Row label="Status" value={<span className="inline-flex items-center gap-1.5">
              ...
              {isDemoLocked && subscription?.message && (<p className="text-[12px] text-amber-300/80 bg-amber-400/10 border border-amber-400/20 rounded-xl px-3 py-2 mb-6 text-left">
                  {subscription.message}
                </p>)}

              <Button onClick={() => navigate("/projects")} className="w-full gap-2 bg-primary text-primary-foreground hover:bg-primary/90 rounded-full h-11 text-sm font-medium shadow-[var(--shadow-glow)]">
              ...
              <div className="flex flex-col gap-3">
                <Button onClick={() => poll()} className="w-full gap-2 bg-primary text-primary-foreground hover:bg-primary/90 rounded-full h-11 text-sm font-medium">
                  <RefreshCw className="w-4 h-4"/> Refresh status
                </Button>
                <Button variant="outline" onClick={() => navigate("/projects")} className="w-full rounded-full h-11 text-sm">
                  Back to dashboard
                </Button>
              </div>
            </>)}

          {sessionId && (<p className="text-[11px] text-muted-foreground/60 mt-6 font-mono truncate">
              Session: {sessionId}
            </p>)}
        </div>
      </div>
...
function cnDot(active) {
    return ("w-2 h-2 rounded-full transition-colors " +
