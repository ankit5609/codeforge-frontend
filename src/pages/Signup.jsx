...
import { FieldEmail, FieldPassword, FieldShell, SolidButton, OutlineButton, QuietButton, Divider, GoogleIcon, } from "@/components/LoginModal";
...
export default function Signup() {
    ...
    const tracker = (<div className="flex items-center flex-wrap gap-2">
      {steps.map((s, i) => (<div key={s.n} className="flex items-center gap-2">
          <div className="flex items-center gap-2">
            <span className="font-mono flex items-center justify-center rounded-md" style={{
                width: 22,
                height: 22,
                fontSize: 11,
            ...
            <span className="text-[13px]" style={{ color: s.active ? "var(--lp-ink)" : "var(--lp-ink-dim)", fontWeight: s.active ? 600 : 400 }}>
              {s.label}
            </span>
          </div>
    ...
    return (<AuthLayout eyebrow="// Create your workspace" heading={<>
        ...
        </>} description="Create your account, then choose a plan to start building." badges={["free to start", "no credit card", "cancel anytime"]} aside={tracker}>
      <AuthCard>
        <form onSubmit={handleSubmit} className="space-y-[16px]">
          <div>
            <label htmlFor="name" className="block font-mono uppercase mb-2" style={{ fontSize: "10.5px", letterSpacing: "0.12em", color: "var(--lp-ink-faint)" }}>
              Name
            </label>
            <FieldShell icon={<User className="w-4 h-4"/>}>
              <input id="name" type="text" autoComplete="name" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} disabled={isLoading} className="lp-input"/>
            </FieldShell>
          </div>
          ...
          <div>
            <FieldPassword value={password} onChange={setPassword} disabled={isLoading} show={showPassword} onToggleShow={() => setShowPassword((s) => !s)} autoComplete="new-password"/>
            <div className="mt-2.5">
              <div className="flex gap-1.5">
                {[0, 1, 2, 3].map((i) => (<span key={i} className="flex-1 h-1 rounded-full transition-colors" style={{
                background: i < strength ? STRENGTH[strength].color : "rgba(255,255,255,0.06)",
            }}/>))}
              </div>
              <div className="flex items-center justify-between mt-1.5 font-mono" style={{ fontSize: 11, color: "var(--lp-ink-faint)" }}>
                <span>{password ? STRENGTH[strength].label : "at least 8 characters"}</span>
                {password && strength >= 3 && (<span className="flex items-center gap-1" style={{ color: "var(--lp-teal)" }}>
