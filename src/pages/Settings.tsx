...
import {
    ArrowLeft, CreditCard, LogOut, Loader2, User as UserIcon, ShieldCheck, Sparkles, Check, Zap,
    Settings as SettingsIcon, Sparkle, FolderGit2, HardDrive, Rocket, MessageSquare, AlertTriangle,
} from "lucide-react";
...
import { deriveTokenUsage, deriveProjectUsage, type DerivedUsage } from "@/lib/usage";
...
/* ============================================================================
   One file, same as the Dashboard pass: everything (helpers, the plan dialog,
   and the page) lives here. All state/handlers from the original Settings.tsx
   are unchanged — the one addition is a second, independent, read-only
   api.getProjects() call (already used elsewhere in the app) so the "Projects"
   usage card can show a real count, since the brief asked for that one
   specifically (unlike Storage/Deployments/Messages, which it explicitly
   marked "(placeholder)"). Flagged again in the summary below.
============================================================================ */
...
function ProgressRing({ pct, size = 44, stroke = 4, color, unlimited }: { pct: number; size?: number; stroke?: number; color: string; unlimited?: boolean }) {
...
function fadeUp(delay = 0) {
    return { initial: { opacity: 0, y: 12 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.45, ease: [0.16, 0.8, 0.3, 1] as const, delay } };
}
...
function MetricCard({ icon: Icon, label, tone, value, meta, pct, unlimited, caption, comingSoon }: {
    icon: typeof CreditCard; label: string; tone: Tone;
    value?: React.ReactNode; meta?: string; pct?: number; unlimited?: boolean; caption?: string; comingSoon?: boolean;
}) {
    ...
    return (
        <motion.div
            ...
            style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)", opacity: comingSoon ? 0.72 : 1 }}
        >
            <div className="flex items-center justify-between">
                {typeof pct === "number" ? (
                    ...
                    <div className="w-9 h-9 rounded-[10px] flex items-center justify-center" style={{ background: c.bg, color: c.fg }}>
                        <Icon className="w-[17px] h-[17px]" strokeWidth={1.75} />
                    </div>
                )}
                {comingSoon && (
                    <span className="font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: "var(--lp-bg-raised-2)", color: "var(--lp-ink-faint)" }}>
                        Soon
                    </span>
                )}
            </div>
            <div>
                ...
                {comingSoon ? (
                    <div className="text-[15px] font-semibold" style={{ color: "var(--lp-ink-faint)" }}>—</div>
                ) : (
...
function PlanDialog({ open, onOpenChange, checkoutPlanId, onChoosePlan }: {
    open: boolean; onOpenChange: (open: boolean) => void; checkoutPlanId: number | null; onChoosePlan: (planId: number) => void;
}) {
    return (
            ...
            <DialogContent className="landing-scope sm:max-w-2xl rounded-[18px] p-6" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
                    {PRICING_PLANS.map((tier) => {
                        ...
                        return (
                            <div key={tier.id} className="relative rounded-[16px] p-6 flex flex-col" style={{ background: "var(--lp-bg-raised-2)", border: tier.isPopular ? "1px solid var(--lp-ember)" : "1px solid var(--lp-border)", boxShadow: tier.isPopular ? "0 20px 50px -24px var(--lp-ember-glow)" : "none" }}>
...
export default function Settings() {
    ...
    const handleBilling = async () => {
        if (!display.isActive) { setIsPlanDialogOpen(true); return; }
    ...
    return (
        <div className="landing-scope min-h-screen w-full" style={{ background: "var(--lp-bg)" }}>
            ...
            <header className="sticky top-0 z-30 backdrop-blur-md" style={{ borderBottom: "1px solid var(--lp-border-soft)", background: "rgba(10,13,18,0.82)" }}>
                <div className="max-w-[1080px] mx-auto flex items-center gap-3 px-5 sm:px-8 h-16">
                    <button onClick={() => navigate("/projects")} aria-label="Back to projects" className="w-9 h-9 flex items-center justify-center rounded-full transition-colors shrink-0"
                        style={{ color: "var(--lp-ink-dim)" }} onMouseEnter={(e) => (e.currentTarget.style.background = "var(--lp-bg-raised)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
            ...
            <main className="relative z-10 max-w-[1080px] mx-auto px-5 sm:px-8 py-9 space-y-7">
                ...
                <motion.section {...fadeUp(0.1)}>
                    ...
                    {loading ? (
                        <div className="rounded-[18px] p-6 space-y-3" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                        ...
                        <div className="rounded-[18px] p-8 flex flex-col items-center text-center gap-3" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                        ...
                        <div className="rounded-[18px] p-5 sm:p-6" style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
                            ...
                            {sub?.message && (
                                <div className="flex items-start gap-3 mt-4 rounded-[14px] p-4" style={{ border: "1px solid rgba(232,184,75,0.3)", background: "rgba(232,184,75,0.07)" }}>
                ...
                <motion.section {...fadeUp(0.15)}>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                        <MetricCard
                            icon={Sparkle} label="AI tokens" tone="ember"
                            value={tokenUsage.unlimited ? "Unlimited" : <AnimatedNumber value={tokenUsage.pct} formatter={(n) => `${n}%`} />}
                            pct={tokenUsage.unlimited ? 100 : tokenUsage.pct} unlimited={tokenUsage.unlimited}
                            caption={tokenUsage.unlimited ? "Your plan includes unlimited AI." : "Resets each billing cycle."}
                        />
                        <MetricCard
                            icon={FolderGit2} label="Projects" tone="brass"
                            value={projectUsage ? <AnimatedNumber value={projectUsage.used} /> : "—"}
                            meta={projectUsage && projectUsage.limit > 0 ? `/ ${projectUsage.limit}` : undefined}
                            pct={projectUsage && projectUsage.limit > 0 ? projectUsage.pct : undefined}
