...
export type RoleFilter = "all" | ProjectRole;
...
const menuContentClass =
  "landing-scope min-w-[176px] rounded-[12px] p-1.5 z-50";
      ...
      <div className="flex items-center gap-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className={menuBtn}
              style={{ border: "1px solid var(--lp-border)", color: "var(--lp-ink-dim)", background: "var(--lp-bg-raised)" }}
            >
          ...
          <DropdownMenuContent align="end" className={menuContentClass} style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}>
            {(Object.keys(SORT_LABEL) as SortOption[]).map((opt) => (
              <DropdownMenuItem
                ...
                className="cursor-pointer rounded-[8px] text-[13.5px] flex items-center justify-between"
        ...
        <div
          className="flex items-center rounded-[10px] p-1 gap-1"
          style={{ border: "1px solid var(--lp-border)", background: "var(--lp-bg-raised)" }}
        >
          <button
            aria-label="Grid view"
            onClick={() => onViewModeChange("grid")}
            className="w-9 h-9 rounded-[7px] flex items-center justify-center transition-colors"
            style={{
