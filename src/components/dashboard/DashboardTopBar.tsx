...
const PLAN_DOT: Record<SubscriptionDisplay["tone"], string> = {
...
export function DashboardTopBar({
  ...
  return (
    <header
      ...
      style={{ borderBottom: "1px solid var(--lp-border-soft)", background: "rgba(10,13,18,0.82)" }}
    >
      <div className="lp-container h-[68px] flex items-center justify-between !px-6 lg:!px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className="hidden sm:inline-flex items-center gap-1.5 text-[12px] font-semibold px-2.5 py-1.5 rounded-full font-mono"
            style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)", color: "var(--lp-ink-dim)" }}
          >
          ...
          <button
            ...
            onMouseEnter={(e) => (e.currentTarget.style.background = "var(--lp-bg-raised)")}
          ...
          {showUpgrade || isDemoLocked ? (
            <button onClick={onUpgradeClick} className="lp-btn lp-btn-solid !h-9 !px-4 !text-[13.5px]">
              <Sparkles className="w-[15px] h-[15px]" /> {isDemoLocked ? "View plans" : "Upgrade"}
            </button>
          ) : isSubscribed ? (
            <button onClick={onManageBilling} disabled={isPortalLoading} className="lp-btn lp-btn-ghost !h-9 !px-4 !text-[13.5px]">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="landing-scope w-72 p-0 overflow-hidden rounded-[14px] z-50"
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid var(--lp-border-soft)" }}>
                ...
                <span
                  ...
                  style={{ color: PLAN_DOT[planDisplay.tone], borderColor: "var(--lp-border)" }}
                >
