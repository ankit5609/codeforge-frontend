...
export function EmptyProjectsState({ isSearching, onCreateProject }: EmptyProjectsStateProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, ease: [0.16, 0.8, 0.3, 1] }}
      className="text-center py-16 px-8 rounded-[20px] max-w-lg mx-auto"
      style={{ border: "1px dashed var(--lp-border)", background: "var(--lp-bg-raised)" }}
    >
      ...
      {!isSearching && (
        <button onClick={onCreateProject} className="lp-btn lp-btn-solid mx-auto">
