...
export function ProjectCard({ project, variant = "grid", onOpen, onRename, onDownload, onDelete }: ProjectCardProps) {
  ...
  const avatar = (
    <div
      className="rounded-[14px] flex items-center justify-center shrink-0 overflow-hidden border border-white/10 shadow-md"
      style={{ width: variant === "grid" ? 48 : 44, height: variant === "grid" ? 48 : 44 }}
    >
  ...
  const quickActions = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
        <button
          className="h-8 w-8 rounded-[8px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
      ...
      <DropdownMenuContent
        align="end"
        className="landing-scope min-w-[160px] rounded-[12px] p-1.5 z-50"
  ...
  const roleBadge = project.role && (
    <span
      className="text-[10px] font-semibold uppercase tracking-wider px-2 py-[3px] rounded-full border"
      style={{ background: roleStyle?.bg, color: roleStyle?.fg, borderColor: roleStyle?.border }}
    >
      {project.role}
    </span>
  );
  ...
  if (variant === "list") {
    return (
      <motion.div
        ...
        onClick={onOpen}
        className="group cursor-pointer rounded-[14px] p-4 flex items-center gap-4"
        style={{ background: "var(--lp-bg-raised)", border: "1px solid var(--lp-border)" }}
      >
        {avatar}
        <div className="min-w-0 flex-1">
          ...
          {project.description ? (
            <p className="text-[13px] truncate mt-0.5" style={{ color: "var(--lp-ink-faint)" }}>
              {project.description}
            </p>
          ) : (
            <span className="flex items-center gap-1 text-[12px] mt-0.5" style={{ color: "var(--lp-ink-faint)" }}>
  ...
  return (
    <motion.div
      layout
      whileHover={{ y: -4 }}
      transition={{ duration: 0.2, ease: [0.16, 0.8, 0.3, 1] }}
      onClick={onOpen}
    ...
    >
      ...
      <p
        className={cn(
          "text-[13px] leading-relaxed mt-3.5 line-clamp-2 flex-1",
          !project.description && "italic opacity-60"
        )}
      ...
      <div
        ...
        style={{ borderTop: "1px solid var(--lp-border-soft)" }}
      >
        {roleBadge || <span />}
