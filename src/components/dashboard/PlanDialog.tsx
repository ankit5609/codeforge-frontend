...
export function PlanDialog({ open, onOpenChange, tiers, checkoutPlanId, onChoosePlan }: PlanDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="landing-scope sm:max-w-2xl rounded-[18px] p-6"
          ...
          {tiers.map((tier) => {
            ...
            return (
              ...
              >
                {tier.recommended && (
                  <span
                    className="absolute -top-2.5 left-6 inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider"
                ...
                <ul className="mt-5 space-y-2.5 flex-1">
                  {tier.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2 text-[13.5px]" style={{ color: "var(--lp-ink-dim)" }}>
                ...
                <button
                  onClick={() => onChoosePlan(tier.planId)}
                  disabled={checkoutPlanId !== null}
                  className={tier.recommended ? "lp-btn lp-btn-solid mt-6 !w-full" : "lp-btn lp-btn-ghost mt-6 !w-full"}
                >
