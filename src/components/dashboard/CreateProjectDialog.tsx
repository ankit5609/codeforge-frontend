...
export function CreateProjectDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="landing-scope !w-[min(360px,calc(100vw-32px))] !max-w-none !h-auto !min-h-0 !max-h-[calc(100vh-96px)] !p-4 !gap-0 !rounded-[12px] overflow-hidden"
        style={{
      ...
      >
              ...
              {PROJECT_TEMPLATES.map((tpl) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      if (tpl.id !== "blank" && !projectName.trim()) onProjectNameChange(tpl.name);
                    }}
                    className="group text-left rounded-[8px] px-2 py-1.5 transition-all hover:-translate-y-0.5"
                    style={{ border: "1px solid var(--lp-border)", background: "var(--lp-bg-raised-2)" }}
                    onMouseEnter={(e) => (e.currentTarget.style.borderColor = "var(--lp-ember)")}
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          <button
            ...
            style={{ padding: "0 12px", fontSize: "12px", borderRadius: "8px", height: "32px", minHeight: "32px", lineHeight: "32px" }}
          >
            Cancel
          </button>
          <button
