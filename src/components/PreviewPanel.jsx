...
import { RuntimeErrorAlert } from "@/components/RuntimeErrorAlert";
export function PreviewPanel({ projectId, runtimeError, onDismiss, onFix }) {
    const getPreviewUrlKey = (projId) => `${PREVIEW_URL_KEY}_${projId}`;
    ...
    const waitForPreviewReady = async (url) => {
    ...
    useEffect(() => {
        const onWorkspaceDeploy = () => {
            if (!isDeploying && !isWaitingForServer)
                handleDeploy();
        };
    ...
    return (<div className="flex flex-col h-full" style={{ background: "var(--lp-bg)" }}>
      ...
      <div className="h-12 shrink-0 flex items-center gap-2 px-3" style={{ borderBottom: "1px solid var(--lp-border-soft)", background: "var(--lp-bg-raised)" }}>
        <button aria-label="Refresh preview" onClick={handleRefresh} disabled={!previewUrl || isWaitingForServer} className="h-7 w-7 rounded-md flex items-center justify-center transition-colors disabled:opacity-40" style={{ color: "var(--lp-ink-faint)" }}>
        ...
        <div className="flex items-center gap-1">
          {previewUrl && (<button aria-label="Open preview in new tab" onClick={() => window.open(previewUrl, "_blank")} className="h-7 w-7 rounded-md flex items-center justify-center transition-colors" style={{ color: "var(--lp-ink-faint)" }}>
              <ExternalLink className="w-3.5 h-3.5"/>
            </button>)}
          <button onClick={handleDeploy} disabled={isDeploying || isWaitingForServer} className="lp-btn lp-btn-solid !h-7 !px-3 !text-xs disabled:opacity-60">
            {isDeploying ? (<>
      ...
      <div className="flex-1" style={{ background: "var(--lp-bg)" }}>
        {isWaitingForServer ? (<div className="flex flex-col items-center justify-center h-full text-center p-8 gap-4">
            ...
            <div className="flex gap-1 mt-2">
              {Array.from({ length: 5 }).map((_, i) => (<span key={i} className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "var(--lp-ember)", opacity: 0.5, animationDelay: `${i * 0.15}s` }}/>))}
            </div>
          </div>) : previewUrl && iframeReady ? (<iframe key={`iframe-${iframeKey}`} src={previewUrl} className="w-full h-full border-0" title="Preview" sandbox="allow-scripts allow-same-origin allow-forms allow-popups"/>) : (<StateView icon={Globe} title="No preview running yet" description="Run a preview to launch your app in a live sandbox." action={!isDeploying && !isWaitingForServer
