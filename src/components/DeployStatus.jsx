...
export function DeployStatus({ state, className, pulse = false, }) {
    const { label, fg, bg, border, icon: Icon, spin } = config[state];
    return (<span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition-shadow duration-300", className)} style={{ color: fg, background: bg, borderColor: border, boxShadow: pulse ? `0 0 0 3px ${bg}` : "none" }} aria-live="polite">
