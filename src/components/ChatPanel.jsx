...
function ChatLoadingSkeleton() {
    return (<div className="flex flex-col gap-6 p-5" aria-hidden="true">
...
const looksLikeBugReport = (text) => BUG_WORDS.some((w) => text.toLowerCase().includes(w));
export function ChatPanel({ messages, onSendMessage, isStreaming, isLoading, readOnly }) {
    ...
    return (<div className="flex flex-col h-full" style={{ background: "var(--lp-bg)" }}>
      {/* Panel header */}
      <div className="h-12 shrink-0 flex items-center gap-2 px-4" style={{ borderBottom: "1px solid var(--lp-border-soft)", background: "var(--lp-bg-raised)" }}>
        ...
        {subscription && (<span className="ml-auto inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-mono" style={{ border: "1px solid var(--lp-border)", background: "var(--lp-bg-raised-2)", color: "var(--lp-ink-faint)" }} title="AI tokens used this cycle">
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (<ChatLoadingSkeleton />) : messages.length === 0 ? (<StateView icon={Bot} title="Let's build something" description="Describe the feature, page, or change you have in mind and I'll get to work.">
            {!readOnly && (<div className="flex flex-wrap items-center justify-center gap-2 max-w-sm pt-1">
                {["Build a landing page", "Add a dashboard", "Create a contact form"].map((s) => (<button key={s} type="button" onClick={() => onSendMessage(s)} className="text-xs px-3 py-1.5 rounded-full transition-colors" style={{ border: "1px solid var(--lp-border)", background: "var(--lp-bg-raised)", color: "var(--lp-ink-faint)" }}>
                    {s}
                  </button>))}
                <button type="button" onClick={() => fileInputRef.current?.click()} className="text-xs px-3 py-1.5 rounded-full transition-colors inline-flex items-center gap-1.5" style={{ border: "1px dashed rgba(255,90,46,0.35)", background: "rgba(255,90,46,0.06)", color: "var(--lp-ember)" }}>
                  <Camera className="w-3 h-3"/> Fix a bug or copy a design from a screenshot
                </button>
              </div>)}
          </StateView>) : (<div className="flex flex-col">
            {messages.map((message) => (<MessageItem key={message.id} message={message} isStreaming={isStreaming && message.isStreaming}/>))}
          </div>)}
      ...
      <div className="shrink-0 p-3" style={{ borderTop: "1px solid var(--lp-border-soft)", background: "var(--lp-bg-raised)" }}>
        {isLimitReached && !readOnly ? (<div className="rounded-xl p-4 text-center space-y-3" style={{ border: "1px solid rgba(232,184,75,0.3)", background: "rgba(232,184,75,0.08)" }}>
            ...
            <form onSubmit={handleSubmit} className="relative">
              {imagePreview && (<div className="mb-2 inline-flex items-start gap-2 rounded-lg p-2" style={{ border: "1px solid var(--lp-border)", background: "var(--lp-bg-raised-2)" }}>
              ...
              <Textarea ref={textareaRef} value={input} onChange={handleTextareaChange} onKeyDown={handleKeyDown} placeholder={readOnly
                ? "You have view-only access to this project"
                : image
                    ? "Add a message (optional) — image will be sent"
                    : "Describe what you want to build..."} className="lp-input min-h-[48px] max-h-[200px] !pl-11 !pr-12 resize-none" disabled={isStreaming || readOnly} rows={1}/>
              <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImagePick}/>
              <button type="button" aria-label="Attach image" onClick={() => fileInputRef.current?.click()} disabled={isStreaming || readOnly} className="absolute left-2 bottom-2 h-8 w-8 rounded-lg flex items-center justify-center transition-colors disabled:opacity-40" style={{ color: "var(--lp-ink-faint)" }}>
                <ImagePlus className="w-4 h-4"/>
              </button>
              <button type="submit" aria-label="Send message" disabled={(!input.trim() && !image) || isStreaming || readOnly} className="absolute right-2 bottom-2 h-8 w-8 rounded-lg flex items-center justify-center disabled:opacity-40 transition-colors" style={{ background: "var(--lp-ember)", color: "#160800" }}>
            ...
            <div className="flex items-center justify-between mt-2 px-1">
              ...
              {isStreaming && (<span className="text-xs flex items-center gap-1 font-medium" style={{ color: "var(--lp-ink-faint)" }}>
...
function MessageItem({ message, isStreaming }) {
    ...
    return (<div className="p-5" style={{ borderBottom: "1px solid var(--lp-border-soft)", background: message.role === 'user' ? 'var(--lp-bg-raised)' : 'var(--lp-bg)' }}>
      <div className="max-w-4xl mx-auto">
        {message.role === "user" ? (<div className="flex flex-col items-end gap-2">
            {message.imageUrl && (<div className="flex flex-col items-end gap-1.5">
                {attachmentLabel && (<span className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full" style={{ background: "var(--lp-bg-raised-2)", color: "var(--lp-ink-faint)", border: "1px solid var(--lp-border)" }}>
                    {attachmentLabel}
                  </span>)}
                <AttachmentImage src={message.imageUrl} alt="User attachment"/>
              </div>)}
            {message.content && (<div className="text-sm py-2.5 px-4 rounded-2xl rounded-tr-none max-w-[85%]" style={{ background: "rgba(255,90,46,0.10)", border: "1px solid rgba(255,90,46,0.2)" }}>
                <p className="leading-relaxed whitespace-pre-wrap" style={{ color: "var(--lp-ink)" }}>{message.content}</p>
              </div>)}
            {message.createdAt && (<span className="text-[10px] px-1 uppercase tracking-tight" style={{ color: "var(--lp-ink-faint)" }}>
                {format(new Date(message.createdAt), "HH:mm")}
              </span>)}
          </div>) : (<div className="space-y-4">
            <div className="flex flex-col gap-3">
              {eventsToRender.map((event, idx) => (<ChatEventRenderer key={idx} event={event} isLoading={isStreaming && idx === eventsToRender.length - 1}/>))}
            </div>

            {!message.isStreaming && eventsToRender.length > 0 && (<div className="flex items-center gap-1 pt-2">
