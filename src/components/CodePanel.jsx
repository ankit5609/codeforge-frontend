...
function findFileInTree(files, targetPath) {
    for (const node of files) {
        if (node.path === targetPath)
            return true;
        if (node.children && findFileInTree(node.children, targetPath))
            return true;
    }
    return false;
}
// Storage key helpers
const getTabsKey = (projectId) => `${OPEN_TABS_KEY}_${projectId}`;
const getActiveTabKey = (projectId) => `${ACTIVE_TAB_KEY}_${projectId}`;
export function CodePanel({ projectId, updatedFiles, refreshKey = 0 }) {
    ...
    const handleSelectTab = useCallback((path) => {
        setActiveTab(path);
    }, []);
    const handleFormat = useCallback(async () => {
        if (!activeTab || !isFormattable(activeTab))
            return;
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-1">
        ...
        <ResizablePanel defaultSize={22} minSize={14} maxSize={40}>
          <div className="h-full overflow-y-auto" style={{ borderRight: "1px solid var(--lp-border-soft)", background: "var(--lp-bg-raised)" }}>
            ...
            <FileTree files={files} selectedPath={activeTab} onSelectFile={handleSelectFile} isLoading={isLoadingTree}/>
          </div>
        </ResizablePanel>
          ...
          <div className="flex h-full min-w-0 flex-col">
            ...
            <div className="flex items-center justify-between" style={{ borderBottom: "1px solid var(--lp-border-soft)", background: "var(--lp-bg-raised)" }}>
              <div className="min-w-0 flex-1">
                <FileTabs openTabs={openTabs} activeTab={activeTab} onSelectTab={handleSelectTab} onCloseTab={handleCloseTab}/>
              </div>
              <div className="flex shrink-0 items-center gap-1 px-2">
                {isEdited && (<button onClick={handleToggleDiff} className={cn("h-7 px-2 rounded-md flex items-center gap-1.5 text-xs font-medium transition-colors")} style={{ color: showDiff ? "var(--lp-ember)" : "var(--lp-ink-faint)" }}>
                    <GitCompare className="h-3.5 w-3.5"/>
                    {showDiff ? "Code" : "Diff"}
                  </button>)}
                <button onClick={handleFormat} disabled={!activeTab || !isFormattable(activeTab) || isFormatting || showDiff} className="h-7 px-2 rounded-md flex items-center gap-1.5 text-xs font-medium transition-colors disabled:opacity-40" style={{ color: "var(--lp-ink-faint)" }}>
                  {isFormatting ? (<Loader2 className="h-3.5 w-3.5 animate-spin"/>) : (<Wand2 className="h-3.5 w-3.5"/>)}
                  Format
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-hidden">
              {showDiff ? (<DiffViewer oldValue={baseline} newValue={fileContent}/>) : (<CodeEditor content={fileContent} filePath={activeTab} isLoading={isLoadingFile}/>)}
            </div>
          </div>
        </ResizablePanel>
      </ResizablePanelGroup>
