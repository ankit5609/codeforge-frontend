...
const wrap = (children: React.ReactNode) => (
  <svg viewBox="0 0 16 16" className="w-4 h-4 shrink-0" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    {children}
  </svg>
);
...
const ReactIcon = ({ color = "#61dafb" }: { color?: string }) =>
  wrap(
...
const TailwindIcon = () =>
  wrap(
    <path
      d="M8 4.3c-1.6 0-2.6.8-3 2.4.6-.8 1.3-1.1 2.1-.9.46.11.78.44 1.14.8C8.94 7.2 9.6 7.9 11 7.9c1.6 0 2.6-.8 3-2.4-.6.8-1.3 1.1-2.1.9-.46-.11-.78-.44-1.14-.8C10.06 5 9.4 4.3 8 4.3zM5 7.9c-1.6 0-2.6.8-3 2.4.6-.8 1.3-1.1 2.1-.9.46.11.78.44 1.14.8C5.94 10.8 6.6 11.5 8 11.5c1.6 0 2.6-.8 3-2.4-.6.8-1.3 1.1-2.1.9-.46-.11-.78-.44-1.14-.8C7.06 8.6 6.4 7.9 5 7.9z"
      fill="#38bdf8"
    />,
  );
...
const FileGenericIcon = ({ color = "#90a4ae" }: { color?: string }) =>
  wrap(
    <path
      d="M3.5 1.5h6L13 5v9a1 1 0 01-1 1H3.5a1 1 0 01-1-1V2.5a1 1 0 011-1z M9.3 1.8V5h3.2"
      fill="none"
      stroke={color}
      strokeWidth="1.1"
...
export function FileTypeIcon({ name }: { name: string }) {
  ...
  if (lower === "bun.lockb" || lower.endsWith(".lock") || lower === "yarn.lock" || lower === "pnpm-lock.yaml") return <LockIcon />;
  if (lower === "dockerfile" || lower.startsWith("dockerfile")) return <DockerIcon />;
  if (lower.startsWith(".gitignore") || lower === ".gitattributes" || lower === ".git") return <GitIcon />;
  if (lower.startsWith(".env")) return <EnvIcon />;
  if (lower.includes("tailwind.config")) return <TailwindIcon />;
  if (lower.includes("postcss.config")) return <PostcssIcon />;
  if (lower.includes("eslint")) return <EslintIcon />;
  if (lower.includes("vitest.config")) return <VitestIcon />;
  if (lower.includes("vite.config") || lower === "vite-env.d.ts") return <ViteIcon />;
  if (lower.startsWith("tsconfig")) return <TsConfigIcon />;
