...
export function ShareDialog({ projectId, trigger, open, onOpenChange }) {
                ...
                <div className="p-6 pb-4">
                    ...
                    <div className="space-y-3 mb-6">
                        <div className="flex gap-2">
                            <Input placeholder="Email or username" className="flex-1 bg-muted/50 border-input/50" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleInvite()}/>
                            <Button onClick={handleInvite} disabled={!inviteEmail.trim() || loading} className="px-6">
                                Invite
                            </Button>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                            {members.length === 0 && (<div className="flex items-center gap-3 p-2 rounded-lg bg-muted/30">
                            ...
                            {members.map(member => (<div key={member.userId} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                                    <Avatar className="h-9 w-9">
                                    ...
                                    {member.role === 'OWNER' ? (<RoleBadge role="OWNER"/>) : (<Select defaultValue={member.role} onValueChange={(val) => {
                    if (val === 'REMOVE')
