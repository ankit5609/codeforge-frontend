...
/**
 * The signature hero demo: a self-driving state machine that cycles
 *   PROMPT → THINKING → READING FILE → EDITING → REBUILDING → PREVIEW LIVE
 * over ~10s, then restarts. All fake/staged content — no network calls.
 */

type Phase = "idle" | "prompt" | "thinking" | "reading" | "editing" | "rebuilding" | "live";
...
const AnimatedDemo = () => {
  ...
  const phase: Phase = useMemo(() => {
    if (reduced) return "live";
    if (t < 400) return "idle";
    if (t < 2200) return "prompt";
    if (t < 3200) return "thinking";
    if (t < 4200) return "reading";
    if (t < 8200) return "editing";
    if (t < 9000) return "rebuilding";
    return "live";
  }, [t, reduced]);
  ...
  const promptProgress = useMemo(() => {
    if (reduced) return 1;
    if (t < 400) return 0;
    if (t < 2200) return Math.min(1, (t - 400) / 1600);
    return 1;
  }, [t, reduced]);
  ...
  const codeShown = useMemo(() => {
    if (reduced) return CODE_LINES.length;
    if (t < 4200) return 0;
    if (t >= 8200) return CODE_LINES.length;
    const p = (t - 4200) / 4000;
  ...
  return (
    <div className="relative w-full">
      ...
      <div
        aria-hidden
        className="absolute -inset-8 rounded-[32px] blur-3xl opacity-70 pointer-events-none"
        style={{
          background:
            "radial-gradient(45% 55% at 30% 40%, rgba(255,90,46,0.18) 0%, transparent 70%), radial-gradient(45% 55% at 80% 80%, rgba(69,196,184,0.14) 0%, transparent 70%)",
        }}
      />
      ...
      >
        ...
        <div
          ...
          style={{ borderBottom: "1px solid var(--lp-border)", background: "rgba(10,13,18,0.6)" }}
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] h-[460px]">
          ...
          <div className="p-5 flex flex-col gap-3 h-full overflow-hidden" style={{ borderRight: "1px solid var(--lp-border)" }}>
            ...
            {/* User bubble */}
            <motion.div
              key={`prompt-${Math.floor(t / LOOP_MS)}`}
              initial={{ opacity: 0, y: 6 }}
              animate={{ opacity: promptProgress > 0 ? 1 : 0, y: 0 }}
            ...
            <div className="flex flex-col gap-2 mt-1">
              <AnimatePresence>
                {showThinking && (
                  <EventChip key="think" tone="brass" label="Thought for 2s" icon="⏳" />
                )}
                {showReading && (
                  <EventChip key="read" tone="teal" label="Reading src/components/PricingTable.tsx" icon="📖" />
                )}
                {showEditing && (
                  <EventChip key="edit" tone="ember" label="Editing PricingTable.tsx" icon="✎" pulse={phase === "editing"} />
                )}
                {showRebuild && (
                  <EventChip key="build" tone="brass" label="Rebuilding preview" icon="⚡" pulse={phase === "rebuilding"} />
                )}
              </AnimatePresence>
            </div>
            ...
            <AnimatePresence>
              {showLive && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="mt-auto pt-3 text-[13px]"
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 flex flex-col min-h-0" style={{ background: "#0B0E14" }}>
              <div
                className="px-4 py-2 text-[11px] font-mono uppercase tracking-wider"
                style={{
              ...
              <div
                className="p-4 flex-1 font-mono text-[12.5px] leading-[1.7] overflow-hidden"
                style={{ fontFamily: "JetBrains Mono, monospace", color: "var(--lp-ink)" }}
              >
                {CODE_LINES.slice(0, codeShown).map((line, i) => (
                  <motion.div
                    key={`${Math.floor(t / LOOP_MS)}-${i}`}
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span
                        ...
                        style={{ color: "var(--lp-ink-dim)", fontFamily: "JetBrains Mono, monospace" }}
                      >
                        Live Preview
                      </span>
                    </div>
...
const EventChip = ({
  ...
  label: string;
  icon: string;
  tone: "ember" | "teal" | "brass";
  pulse?: boolean;
}) => {
  const color =
    tone === "ember" ? "var(--lp-ember)" : tone === "teal" ? "var(--lp-teal)" : "var(--lp-brass)";
  return (
    <motion.div
      initial={{ opacity: 0, y: 6 }}
      animate={{ opacity: 1, y: 0 }}
      ...
      className="inline-flex items-center gap-2 self-start px-3 py-1.5 rounded-full text-[12px] font-medium"
      style={{
...
const PreviewTier = ({ name, price, active }: { name: string; price: string; active?: boolean }) => (
  <div
  ...
  >
    <div
      ...
      style={{ color: active ? "var(--lp-ember)" : "var(--lp-ink-dim)", fontFamily: "JetBrains Mono, monospace" }}
    >
      {name}
    </div>
    <div
      className="text-[15px] font-bold"
      style={{ color: "var(--lp-ink)", fontFamily: "Bricolage Grotesque, sans-serif" }}
    >
      {price}
    </div>
  </div>
