...
const WorkflowSection = () => {
  ...
  return (
    <section id="how-it-works" className="lp-section relative">
      <div className="lp-container">
        <SectionHeader
          center
          eyebrow="// The loop"
          title={<>From a sentence to a running app.</>}
          subtitle="Every turn cycles through the same four beats — and you see each one happen."
        />
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-2 relative">
          ...
          <div
            aria-hidden
            className="hidden md:block absolute left-0 right-0 top-[42px] h-px"
            style={{ background: "var(--lp-border)" }}
          />
          <motion.div
            aria-hidden
            className="hidden md:block absolute top-[42px] h-px"
            style={{ background: "linear-gradient(90deg, var(--lp-ember), var(--lp-teal))" }}
            initial={false}
            animate={{
              left: "0%",
