  ...
  <section id="pricing" className="lp-section">
    <div className="lp-container">
      <SectionHeader
        center
        eyebrow="// Pricing"
        title={<>Two plans. Both get the full workspace.</>}
        subtitle="No feature-gated tiers and no free plan — every subscription includes the complete AI workspace, live sandboxes included."
      />
        ...
        {PRICING_PLANS.map((plan, i) => {
          ...
          return (
            <motion.div
              ...
              transition={{ duration: 0.55, delay: i * 0.1, ease: [0.16, 0.8, 0.3, 1] }}
            ...
            >
              {featured && (
                <div
                  className="absolute -top-3 left-8 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider"
                  style={{
              ...
              <div
                ...
                style={{ color: featured ? "var(--lp-ember)" : "var(--lp-brass)", fontFamily: "JetBrains Mono, monospace" }}
              >
                {plan.description}
              </div>
              ...
              <ul className="flex flex-col gap-3 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-[14.5px]" style={{ color: "var(--lp-ink)" }}>
                    <Check
                      className="w-4 h-4 mt-1 flex-shrink-0"
                      style={{ color: featured ? "var(--lp-ember)" : "var(--lp-teal)" }}
                      strokeWidth={2.5}
                    />
                    {f}
                  </li>
              ...
              <Link
                to="/login"
                className={`lp-btn lp-btn-block ${featured ? "lp-btn-solid" : "lp-btn-ghost"}`}
              >
