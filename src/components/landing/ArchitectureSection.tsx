...
const ArchitectureSection = () => {
  ...
  return (
    <section id="infrastructure" className="lp-section-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[0.9fr_1.2fr] gap-10 lg:gap-14 items-center">
          ...
          <div>
            <SectionHeader
              ...
              title={<>Real distributed infrastructure — not a shared playground.</>}
              subtitle="Every project runs in its own Kubernetes pod. Independent services sit behind a single gateway, built to hold up under real, concurrent use."
            />
              ...
              {[
                ...
                >
                  <span
                    ...
                    style={{ background: "var(--lp-ember)", boxShadow: "0 0 8px var(--lp-ember-glow)" }}
                  />
                  <div>
          ...
          <motion.div
            ...
            style={{
              background:
                "linear-gradient(180deg, rgba(18,22,29,0.9) 0%, rgba(10,13,18,0.9) 100%)",
...
const ArchRow = ({ children }: { children: React.ReactNode }) => (
...
const ArchNode = ({
  ...
  const accent =
    tone === "ember" ? "var(--lp-ember)" : tone === "teal" ? "var(--lp-teal)" : tone === "brass" ? "var(--lp-brass)" : "var(--lp-ink-dim)";
  return (
    <div
      className={`rounded-lg px-4 py-3 flex-1 max-w-[220px] text-center ${big ? "py-4" : ""}`}
      style={{
    ...
    >
      <div
        className={`text-[${big ? 15 : 14}px] font-semibold`}
        style={{
          color: "var(--lp-ink)",
...
const Connector = ({ pulse, keyId, small }: { pulse: number; keyId: string; small?: boolean }) => (
  <div className={`relative mx-auto my-2 ${small ? "h-6" : "h-8"} w-px`} style={{ background: "var(--lp-border)" }}>
    <motion.div
      key={`${keyId}-${pulse}`}
      ...
      style={{ background: "var(--lp-ember)", boxShadow: "0 0 10px var(--lp-ember-glow)" }}
    />
  </div>
);
