...
const WhyCodeForge = () => (<section id="why" className="lp-section">
    <div className="lp-container">
      <SectionHeader eyebrow="// Traditional vs CodeForge" title={<>The old way was designed<br />before AI could actually write code.</>} subtitle="Every part of a modern build changes when the tooling is agent-first."/>

      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className="mt-12 rounded-2xl overflow-hidden" style={{
        background: "var(--lp-bg-raised)",
        border: "1px solid var(--lp-border)",
    }}>
        {/* header row */}
        <div className="grid grid-cols-[1fr_1fr_1.15fr] items-center px-6 py-4 text-[12px] font-mono uppercase tracking-wider" style={{
        ...
        {ROWS.map((r, i) => (<motion.div key={r.label} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }} className="grid grid-cols-[1fr_1fr_1.15fr] items-center px-6 py-5 gap-4" style={{
