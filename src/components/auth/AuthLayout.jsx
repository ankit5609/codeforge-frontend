...
const BrandMark = () => (<Link to="/" className="flex items-center gap-2.5 font-bold text-[17px] tracking-tight" style={{ fontFamily: "'Bricolage Grotesque', sans-serif", color: "var(--lp-ink)" }} aria-label="CodeForge home">
...
export function AuthLayout({ eyebrow, heading, description, badges, aside, children }) {
    return (<div className="landing-scope min-h-screen lg:h-screen w-full flex flex-col relative overflow-hidden">
      ...
      <div aria-hidden className="absolute inset-0 pointer-events-none z-0" style={{
            background: "radial-gradient(60% 40% at 10% 15%, rgba(255,90,46,0.10) 0%, transparent 60%), radial-gradient(50% 40% at 95% 25%, rgba(69,196,184,0.08) 0%, transparent 60%)",
        }}/>
      ...
      <main className="relative z-10 flex-1 min-h-0 w-full">
        <div className="mx-auto h-full max-w-[1280px] px-6 sm:px-10 py-8 lg:py-6 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] gap-10 lg:gap-14 items-center">
          ...
          <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 0.8, 0.3, 1] }} className="max-w-[540px]">
            ...
            <h1 className="text-[clamp(30px,3.6vw,44px)] leading-[1.05] font-bold tracking-[-0.02em] mb-4" style={{ fontFamily: "'Bricolage Grotesque', 'Segoe UI', sans-serif" }}>
              {heading}
            </h1>
            ...
            {badges && badges.length > 0 && (<div className="flex flex-wrap items-center gap-x-2 gap-y-2 font-mono text-[12px]" style={{ color: "var(--lp-ink-faint)" }}>
                <span className="lp-live-dot inline-block w-1.5 h-1.5 rounded-full"/>
                {badges.map((b, i) => (<span key={b} className="flex items-center gap-2">
                    <span>{b}</span>
...
export function AuthCard({ children }) {
    return (<div className="relative rounded-[18px] p-6 sm:p-8 overflow-hidden" style={{
            background: "var(--lp-bg-raised)",
            border: "1px solid var(--lp-border)",
            boxShadow: "0 40px 90px -44px rgba(0,0,0,0.7)",
        }}>
      <div aria-hidden className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: "linear-gradient(120deg, #FF5A2E 0%, #FF8A5E 45%, #E8B84B 100%)" }}/>
      {children}
    </div>);
}
