...
export function DiffViewer({ oldValue, newValue, className }) {
    const rows = useMemo(() => diffLines(oldValue.split("\n"), newValue.split("\n")), [oldValue, newValue]);
    return (<div className={cn("h-full overflow-auto font-mono text-xs leading-relaxed", className)} style={{ background: "var(--lp-bg)" }}>
          ...
          {rows.map((row, idx) => {
            ...
            const markerColor = row.type === "add"
                ...
                : row.type === "del"
                    ? "var(--lp-ember)"
                    : "var(--lp-ink-faint)";
            return (<tr key={idx} style={{ background: bg }}>
