...
export const api = {
    ...
    async getFileContent(projectId, path) {
        const response = await fetch(`${BASE_URL}/api/v1/workspace/projects/${projectId}/files/content?path=${encodeURIComponent(path)}`, {
            headers: { ...getAuthHeaders() },
        });
        if (!response.ok) {
    ...
    async streamChat(projectId, message, onChunk, onFile, onComplete, onError, image) {
        ...
        fetch(`${BASE_URL}/api/v1/intelligence/chat/stream`, {
            ...
            if (!response.ok) {
                ...
                if (response.status === 400)
                    msg = "Please enter a message or attach an image.";
                else if (response.status === 403)
                    msg = "Daily token limit reached or no access to this project.";
                try {
     ...
     */
    async fetchAttachment(relativePath) {
